...
exists = path.exists("/home/yogi/Documents/Python-Requests-API/file.json")
if exists:
...
def exercises():
    user = int(input("enter an index input to choose among the list of courses: "))
    z = ""
    for i in range(len(l)):
        if user == i:
    ...
    user2 = int(input("enter another input to choose among the list of exercises: ")) # to choose among the list of Courses
    print ("")
    for i in range(len(z)):
        if user2 <= j: # to check if input given is wrong. Here `j` has its maximum value
            if user2 == i:
                ...
                print (access + "\n")
                while True:
                    agla_input = input("enter an input(`p` for previous slug or `n` for next slug or `up` for main-menu or `e` to exit): ")
                    if agla_input == "p":
                        i = i-1
                        if i < 0:
